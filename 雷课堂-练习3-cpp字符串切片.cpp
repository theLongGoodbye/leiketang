...
zfill(int n, int width) {
    ...
    int n_str_length = string(n_str).length();
    int substract = width - n_str_length;
    string fill = nChar(substract);
    string result = fill + n_str;
    return result;
...
rjust(const string &s, int width, char fillchar=' ') {
    ...
    // 4. 拼接并返回结果
    int n_str_length = string(s).length();
    int substract = width - n_str_length;
    string fill = nChar(substract, fillchar);
...
center(const string &s, int width, char fillchar=' ') {
    ...
    int n_str_length = string(s).length();
    int substract = width - n_str_length;
    int l_len = substract/2;
    int r_len = substract - l_len;
    string l_str = nChar(l_len, fillchar);
    string r_str = nChar(r_len, fillchar);

    string result = l_str + s + r_str;
    return result;
}
...
stripLeft(const string &s) {
    ...
    while(i < s.size()) {
        if (s[i] != ' ') {
            int s_len = string(s).length();
            return s.substr(i, s_len - i);
        }
        i = i +1;
    }
    return s;
...
strip(const string &s) {
    ...
    // 3. 返回结果
    string s_stripLeft = stripLeft(s);
    string s_stripRight = stripRight(s_stripLeft);
    return s_stripRight;
}
...
replace(const string &s, const string &old_str, const string &new_str) {
    ...
    // 3. 拼接并返回结果
    int old_str_len = string(old_str).length();
    int s_len = string(s).length();
    int i = 0;
    while(i < s.size()) {
        auto offset = old_str_len;
        auto slice_str = s.substr(i, offset);
        log("slice_str", slice_str);
        if (slice_str == old_str) {
            
            string l_s = s.substr(0, i);
            string r_s = s.substr(i + offset, s_len - i);
            log('l', l_s);
            log('r', r_s);
            return l_s + new_str + r_s;
        }
        i = i + 1;
    }
    return s;
